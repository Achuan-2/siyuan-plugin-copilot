## v2.8.2 / 20260906
- 🎨 适配思源v3.8.3
- 🎨 模型自定义最大 Token 参数，上下文占用要正确显示最大token

## v2.8.0 / 20260821
- 🎨 平台管理支持模型排序
- 🎨 手机端优化：支持复制对话、支持划选复制部分内容
- 🎨 手机端优化：平台和模型支持排序
- 🎨 点击新窗口选择打开页签，要自动打开当前聊天会话，而不是空界面，还要自行选择会话
- 🎨 新建会话不影响当前正在进行会话
- 🎨 添加赞助商 APIMart
...
- 🎨 侧栏添加最小化按钮
- 🔥 移除侧栏顶部冗余按钮
- 🔥 移除联网按钮，统一使用思源提供的搜索工具，不再对模型进行单独适配

## v2.7.6 / 20260728
- 🐛 问答模式启用工具时，拖入块没有传递上下文具体内容，只传入id

## v2.7.5 / 20260724
- 🎨 小程序集合侧栏：网页小程序点击网页链接，也在集合侧栏打开
- 🐛 手机端滑动聊天记录会错误收起侧栏

## v2.7.3 / 20260719
- 🎨 使用protyle当编辑器
- 🎨 思源最低版本要求：v3.7.2
- 🎨 思源MCP工具列表优化：优化插件工具显示

## v2.7.2  / 20260717
- 🎨 小程序集合侧栏：只显示顶部的全屏按钮，移除每个小程序单独的全屏按钮
- 🎨 小程序集合侧栏：支持管理小程序
- 🐛 生图模式目前无法选择模型，问答模式选了模型，切换到生图模式，始终为问答模式之前的模型，怎么修改都无法修改为画图模型
- ♻️ 开发者：pnpm build也要把dist文件复制到思源插件文件夹


## v2.7.1 / 20260712
- 🎨 工具选择对话框优化：顶部添加搜索框，可以搜索工具，并添加一个筛选按钮，可以快速筛选已选择工具


## v2.7.0 / 20260712
- ✨ 新增侧栏小程序集合dock，可统一在一个dock打开所有小程序
- 🌐 开发者：改进i18n，取消父子json结构，变为平铺，i18n键使用驼峰拼写，这样方便直接根据i18n键就能快速找到对应翻译

## v2.6.0 / 20260711
- ✨ 思源相关工具替换为思源官方MCP工具
- ✨ 支持加载任务笔记管理插件的工具调用skill
- 🎨 问答、agent、画图模式提示词区别
- 🎨 手机端打开平台管理交互优化
- 🎨 优化不同模型的思考程度设置
- 🎨 内置平台支持编辑官网链接和名称
- 🎨 生图模式仅生成文字时优化，输出生成的文字
- 🎨 生图模式选择的模型单独记忆，切换到其他模式，不保留
- 🐛 当问答模式多模型对话，切换到agent模式单模型的时候，显示的上下文用量依然错误显示多个模型而不是仅显示一个模型
- 🐛 预设选择模型，如果清空了模型，依然会错误在模型列表中原先的模型左边显示数字角标1
- 🐛 注册小程序到侧栏，每次启动思源都错误自动打开小程序侧栏
